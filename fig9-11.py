...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Train Emo F1: {tr[1]:.3f} | Val Emo F1: {va[1]:.3f}")
    ...
    with torch.no_grad():
        for text, audio, img, mask, emo, sent in test_loader:
            ...
            emo_prob = F.softmax(emo_out, dim=1)

            all_emo_true.extend(emo.numpy())
            all_sent_true.extend(sent.numpy())
            all_emo_pred.extend(torch.argmax(emo_out, 1).cpu().numpy())
            all_sent_pred.extend(torch.argmax(sent_out, 1).cpu().numpy())
            all_emo_probs.append(emo_prob.cpu())

    all_emo_probs = torch.cat(all_emo_probs)


    plt.figure(figsize=(10,6))
    counts = np.bincount(all_emo_pred)
    plt.bar(emotion_encoder.classes_, counts)
    plt.xticks(rotation=45)
    plt.title("Distribution of Predicted Emotion Classes")
    plt.tight_layout()
    plt.savefig("Figure9_Emotion_Distribution.png", dpi=300)
    plt.show()

 
    cm = confusion_matrix(all_sent_true, all_sent_pred)
    ...
    plt.tight_layout()
    plt.savefig("Figure10_Sentiment_Confusion.png", dpi=300)
    plt.show()

  
    sent_classes = sentiment_encoder.classes_
    emo_classes = emotion_encoder.classes_

    avg_probs = []
    for i in range(len(sent_classes)):
        idx = np.array(all_sent_true) == i
        avg_prob = all_emo_probs[idx].mean(dim=0)
        avg_probs.append(avg_prob.numpy())
    ...
    for i in range(len(emo_classes)):
        plt.bar(x + i*width, avg_probs[:, i], width, label=emo_classes[i])

    plt.xticks(x + width*3, sent_classes)
    plt.ylabel("Predicted Probability")
    plt.title("Predicted Emotion Probability vs Sentiment")
    plt.legend(bbox_to_anchor=(1.05,1))
    plt.tight_layout()
    plt.savefig("Figure11_Emotion_vs_Sentiment.png", dpi=300)
    plt.show()